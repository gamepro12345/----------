...
pygame.mixer.init()
pyxel.init(160, 120, title="BULLETS SURVIVOR", fps=30)
pyxel.load("assets/bullets_game_edit.pyxres")
# 定数・初期設定
W = 160
H = 120
MP_MAX = 9
...
# BGMパス
BGM_PATHS = {
    1: "assets/famicom_easy.mp3",
    2: "assets/famicom_nomal.mp3",
    3: "assets/famicom_hard.mp3",
    4: "assets/famicom_danger.mp3"
}
...
def update():
    ...
    global PATTERN, PATTERN_TIMER, game_over_timer, title_timer, selected_level
    global player_bullets, BOSS_HP, boss_dead, boss_dead_timer
    ...
    if scene == "title":
        ...
        if pyxel.btnp(pyxel.KEY_RETURN) or pyxel.btnp(pyxel.KEY_Z):
            key_pressed = selected_level
        else:
            key_pressed = None

        if key_pressed is not None:
            # 難易度に応じてHP設定
            if key_pressed == 1: HP = 20
    ...
    if scene == "play":
            ...
            for b in bullets:
                ...
                if -10 <= b[0] <= W + 10 and -10 <= b[1] <= H + 10:
                    ...
                    if (invincible_frames == 0 and not game_over and
                        p_x < b[0] + b[4] and b[0] < p_x  and
                        p_y < b[1] + b[4] and b[1] < p_y):
        ...
        # プレイヤー弾発射
        if pyxel.btnp(pyxel.KEY_Z) and not game_over:
            player_bullets.append([p_x + PLAYER_SIZE // 2, p_y, 0, -3, 3, 10])
        ...
        for pb in player_bullets:
            ...
            # ボスに命中
            if (EMIT_CENTER_X - 4 < pb[0] < EMIT_CENTER_X + 4 and
                EMIT_CENTER_Y - 4 < pb[1] < EMIT_CENTER_Y + 4):
...
def draw():
    global scene, selected_level, title_timer, boss_dead_timer

    # ボス撃破フラッシュ
    if boss_dead and boss_dead_timer % 10 < 5 and not scene == "title":
        pyxel.cls(7)
    else:
        pyxel.cls(0)
    ...
    if scene == "title":
        ...
        levels = ["1: EASY", "2: NORMAL", "3: HARD", "4: DANGER"]
        for i, text in enumerate(levels):
            y = H // 2 + i * 12
            pyxel.text(W // 2 - 20, y, text, 7)

        cursor_y = H // 2 + (selected_level - 1) * 12
        pyxel.text(W // 2 - 30, cursor_y, ">", 10)
        return
    ...
    if invincible_frames > 0 and (invincible_frames // 2) % 2 == 0:
        ...
        pyxel.blt(p_x-5, p_y-5, 0, 0, 0, 16, 16, 11)

    # エミッター（ボス）
    if not boss_dead:
        pyxel.circ(EMIT_CENTER_X, EMIT_CENTER_Y, 3, 9)
    ...
    # プレイヤー弾
    for pb in player_bullets:
        pyxel.rect(pb[0], pb[1], pb[4], pb[4], pb[4])

    # UI
    pyxel.text(5, 5, f"HP: {HP}", 7)
    pyxel.text(5, 15, f"BOSS: {BOSS_HP}", 8)
